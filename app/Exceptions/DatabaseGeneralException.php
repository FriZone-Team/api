...
class DatabaseGeneralException extends AppException implements DatabaseException
{
    public static function build(DatabaseQueryException $ex)
    {
        $code = null;
        $message = null;
        if (preg_match_all('/^General error: ([0-9]+) (.*?)$/', $ex->resultMessage, $matches)) {
            list(, list($code), list($message)) = $matches;
        }
        $ex = new static($ex, $code, $message);
        switch ($code) {
            case '1364':
                return DatabaseMissValueException::build($ex);
        }
        return $ex;
    }
